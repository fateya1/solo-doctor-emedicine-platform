// src/screens/patient/PatientMessages.tsx
...
export default function PatientMessages() {
  ...
  const filtered = conversations.filter(c => {
    const name = c.doctor?.fullName ?? c.doctorProfile?.user?.fullName ?? '';
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.searchBox}>
        ...
        <TextInput style={styles.searchInput} placeholder="Search doctors..." placeholderTextColor="#94a3b8" value={search} onChangeText={setSearch} />
      </View>
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={async () => { setRefreshing(true); await load(); setRefreshing(false); }} />}>
        {filtered.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyHint}>Messages from your doctors will appear here</Text>
          </View>
        ) : (
          filtered.map((conv: any) => {
            const name = conv.doctor?.fullName ?? conv.doctorProfile?.user?.fullName ?? 'Doctor';
