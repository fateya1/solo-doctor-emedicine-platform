                          ...
                          <div className="flex gap-3 mt-1 flex-wrap">
                            ...
                            {lastPayment && (
                              <span className="text-xs text-slate-400">
                                Last payment: KES {Number(lastPayment.amount).toLocaleString()} · {lastPayment.paidAt ? format(new Date(lastPayment.paidAt), "MMM d, yyyy") : lastPayment.status}
                              </span>
                            )}
                          </div>
                        </div>
                          ...
                          {s.payments.map((p: any) => (
                            <div key={p.id} className="flex items-center justify-between text-xs">
                              <span className="text-slate-600">
                                KES {Number(p.amount).toLocaleString()} · {p.plan} · {p.mpesaReceiptNo ?? "No receipt"}
                              </span>
                              <div className="flex items-center gap-2">
                                <span className="text-slate-400">
                                  {p.paidAt ? format(new Date(p.paidAt), "MMM d, yyyy") : p.createdAt ? format(new Date(p.createdAt), "MMM d, yyyy") : "—"}
                                </span>
