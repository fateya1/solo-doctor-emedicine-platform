...
export default function AdminMigrationsPage() {
  ...
  if (loading) return <div className="p-6 text-slate-500">Loading migrations...</div>;
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={s.label} className="bg-white rounded-xl border border-slate-200 p-4 text-center">
            <div className={	ext-3xl font-bold }>{s.value}</div>
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => approve(m.id)} disabled={processing === m.id}
                      ...
                      {processing === m.id ? 'Processing...' : 'Approve & Send Emails'}
                    </button>
                  </div>
                </div>

                {selected?.id === m.id && m.patients && (
