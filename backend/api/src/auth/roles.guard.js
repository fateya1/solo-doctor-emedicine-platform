...
var RolesGuard = function () {
    ...
    var RolesGuard = _classThis = /** @class */ (function () {
        ...
        RolesGuard_1.prototype.canActivate = function (ctx) {
            var requiredRoles = this.reflector.getAllAndOverride(roles_decorator_1.ROLES_KEY, [ctx.getHandler(), ctx.getClass()]);
            if (!requiredRoles || requiredRoles.length === 0)
                return true;
            var req = ctx.switchToHttp().getRequest();
            var user = req.user; // from JwtStrategy validate()
