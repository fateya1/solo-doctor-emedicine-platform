...
export class MigrationService {
  ...
  async uploadMigration(doctorUserId: string, rows: any[], fileName: string) {
    ...
    return {
      message: 'Migration uploaded successfully. Awaiting admin approval.',
  ...
  async approveMigration(migrationId: string, adminUserId: string) {
      ...
      try {
        ...
        if (existing) {
          await this.prisma.migrationPatient.update({
            where: { id: patient.id },
            data: { status: 'FAILED', errorMessage: 'Email already registered' },
          });
          failed++;
          continue;
        }
        ...
        const loginUrl = ${process.env.FRONTEND_URL ?? 'https://solo-doctor-emedicine-platform.vercel.app'}/auth/login;
        await this.emailService.sendMigrationWelcome(
          patient.email,
          patient.fullName,
          migration.doctor.user.fullName,
          patient.email,
          tempPassword,
          loginUrl,
        );
    ...
    await this.prisma.patientMigration.update({
      where: { id: migrationId },
      data: {
        status: failed === migration.patients.length ? 'FAILED' : 'COMPLETED',
